...
/**
 * Random policy - Baseline (without migration)
 *
 ...
 * c) Select one server randomly and save the selected configuration
 *	so that it can be used in random policy - with migration
...
for($x = 0; $x < $p; $x++) {
   ...
   if (!$result) {
        $c0->set("key_".$zipf_array[$x], "value_".$zipf_array[$x]) or die("Couldn't save anything to memcached $x and count is $count...<br />");
   } else {
...
$randNo = 4; //Remove this line later.Used for testing
if ($randNo == 1) {
	$minServer = $server1;
        $currCluster = $c1;
} elseif ($randNo == 2) {
	...
	$minServer = $server4;
        $currCluster = $c4;
} else print "no matching server";

$fh = fopen("/tmp/randNo.txt",'w') or die("cant open output file");
fwrite($fh,$randNo."\n");
fclose($fh);
echo "rand no is ".$randNo;
...
echo "Before migration of hot keys, avg response time = ".$avgResponseTime."<br>";
